...
| 16 | **写/改完成后复查**：打开已改文件，确认未误加空行（逐行插空、成片 `\n\n`、CRLF 规范化翻倍等）；意图外空行须立刻去掉 |
| 17 | 非必要临时变量可简化则简化（仅用一次且无助读时内联） |
| 18 | 具有共通性的代码块之间用**空行**分隔（如：赋值组 / 调用组 / 回调注册） |
| 19 | 有返回值的 `return` 前加**空行**（同块内仅一条语句的 `return` 除外） |
...
- 短 `for` 无故拆成多行
- 调用写成 `foo\n(`（`(` 必须行尾：`foo(`）
- 改完留下「每行后多一空行」或其它非意图空行（换行规范化、批量替换后尤须查）
